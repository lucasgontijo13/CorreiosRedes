...
import java.net.InetAddress; // NOVO: Import para obter o IP
import java.net.ServerSocket;
import java.net.Socket;
import java.net.SocketException;
import java.net.UnknownHostException; // NOVO: Import para tratar exceção de IP
...
public class FtpServer extends JFrame {
    ...
    // MODIFICADO: Lógica de inicialização do servidor
    public void startServer() {
        new Thread(() -> {
            try {
                ...
                // NOVO: Bloco para obter o endereço de IP local
                String ipAddress;
                try {
                    ipAddress = InetAddress.getLocalHost().getHostAddress();
                } catch (UnknownHostException e) {
                    ipAddress = "localhost (não foi possível obter o IP)";
                    System.err.println("Erro ao obter o endereço de IP local: " + e.getMessage());
                }

                // MODIFICADO: A mensagem de inicialização agora inclui o IP
                System.out.println("Servidor FTP iniciado em: " + ipAddress + ":" + PORT);
                ...
                while (running) {
                    try {
                        ...
                        System.out.println("Nova conexão de: " + client.getInetAddress().getHostAddress());
                        pool.submit(new ClientHandler(client, tracking));
                    } catch (SocketException e) {
                        if (running) System.err.println("Erro no socket: " + e.getMessage());
                        else System.out.println("Socket do servidor fechado. Novas conexões não serão aceitas.");
                    } catch (IOException e) {
    ...
    public void shutdown() {
        int response = JOptionPane.showConfirmDialog(
                this,
                "Tem certeza que deseja encerrar o servidor?",
        ...
        System.out.println("------------------------------------");
        System.out.println("Iniciando o encerramento do servidor...");
        running = false;
    ...
    private void loadShipmentsFromDisk(ConcurrentMap<String, ShipmentInfo> trackingMap) {
        ...
        try (Stream<Path> paths = Files.list(uploadsDir)) {
            paths.filter(Files::isRegularFile).forEach(path -> {
                ...
                if (parts.length < 3) {
                    System.err.println(" -> Arquivo ignorado (formato inválido): " + fullFileName);
                    return;
                }
                ...
                try {
                    ...
                    System.out.println(" -> Encomenda carregada: " + shipmentId + " | " + originalFilename + " | Status: " + status);
                } catch (IOException e) {
